...
def _profile(sensor_id: str, mounting_mode: MountingMode) -> CalibrationProfile:
    ...
    return CalibrationProfile(
        ...
        extrinsics=RigidTransform(
            ...
            to_frame=(
                TransformFrame.TEMPLATE_BASE
                if static
                else TransformFrame.ROBOT_FLANGE
            ),
...
def _config(run_root: Path, *, reference_path: str | None) -> dict:
    return create_run_config(
        ...
        robot_pose_sunrise_reference_frame_path=reference_path,
    ).to_dict()
...
def test_static_only_reuse_requires_canonical_destination_config(tmp_path: Path) -> None:
    static = _profile("static", MountingMode.STATIC)

    result = verify_static_profile_destination_reference(
        tmp_path,
        _config(tmp_path, reference_path=POSE_TEMPLATE_BASE_SUNRISE_PATH),
        [static],
    )

    assert result is not None
    assert result["matched_robot_pose_artifacts"] == []
    with pytest.raises(ValueError, match="found 'unconfigured'"):
        verify_static_profile_destination_reference(
            tmp_path,
            _config(tmp_path, reference_path=None),
            [static],
        )
...
def test_mixed_reuse_rejects_unprovenanced_or_wrong_matched_robot_poses(
    ...
    with pytest.raises(ValueError, match="Synchronized robot poses"):
        verify_static_profile_destination_reference(
            tmp_path,
            _config(tmp_path, reference_path=POSE_TEMPLATE_BASE_SUNRISE_PATH),
            [
...
def test_mixed_reuse_accepts_verified_pose_template_base_matches(
    ...
    assert result is not None
    assert result["sunrise_reference_frame_path"] == (
        POSE_TEMPLATE_BASE_SUNRISE_PATH
    )
