...
def get_sensor_adapter(sensor_type: SensorType | str) -> SensorAdapterSpec:
    ...
    try:
        ...
        raise ValueError(f"No capture adapter is registered for {normalized.value}") from exc


def list_sensor_adapters() -> list[dict[str, Any]]:
    return [
        adapter.to_dict()
        for adapter in sorted(SENSOR_ADAPTERS.values(), key=lambda item: item.sensor_type.value)
    ]
...
def build_sensor_capture_command(
    *,
    ...
    if SensorType(sensor_type) == SensorType.ZED_2I:
