...
def _capture_jobs_for_run(run_root: str | Path | None = None) -> list[dict]:
    ...
    for job in job_runner.list():
        ...
        if run_root is not None:
            job_run_root = getattr(
                job,
                "run_root",
                (job.parameters or {}).get("run_root"),
            )
...
def list_jobs():
    ...
    try:
        ...
        return jsonify({"output": "limit must be an integer"}), 400
    ...
    for status in statuses:
        if status == "active":
            expanded_statuses.update(ACTIVE_JOB_STATUSES)
        elif status in {"finished", "terminal"}:
            expanded_statuses.update({"succeeded", "failed", "canceled"})
        else:
            expanded_statuses.add(status)
    raw_scopes = [
        *request.args.getlist("scope_kind"),
        *request.args.getlist("scope"),
    ]
    scopes = {
        item.strip().lower()
    ...
    try:
        page = job_runner.list_page(
            ...
            statuses=expanded_statuses or None,
