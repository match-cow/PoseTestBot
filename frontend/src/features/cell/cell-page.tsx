  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-5">
      ...
      <div className="space-y-4"><Card><CardHeader><CardTitle className="text-base">Visibility layers</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-2">{LAYERS.map((layer) => <Label key={layer} className="flex items-center gap-2 rounded border p-2 text-xs"><Checkbox checked={visible.has(layer)} onCheckedChange={() => toggleLayer(layer)} />{titleCase(layer)}</Label>)}<Label className="col-span-2 flex items-center gap-2 rounded border p-2 text-xs"><Checkbox checked={trajectory} onCheckedChange={(value) => setTrajectory(value === true)} /><Route className="size-3.5" />Recorded trajectory</Label></CardContent></Card>
        ...
        <Card><CardHeader><CardTitle className="text-base">Dataset contents</CardTitle><CardDescription>{scene.object_selection.objectless ? "Explicit objectless RGB-D run" : `${scene.object_selection.selected_objects.length} selected object(s)`}</CardDescription></CardHeader><CardContent className="max-h-80 space-y-2 overflow-auto">{scene.entities.map((entity) => <button key={entity.id} type="button" className="flex w-full items-center gap-2 rounded border px-3 py-2 text-left text-xs hover:bg-muted" onClick={() => setSelected(entity)}>{entity.type === "camera" ? <Camera className="size-3.5" /> : entity.status === "unresolved" ? <EyeOff className="size-3.5 text-destructive" /> : <Eye className="size-3.5" />}<span className="min-w-0 flex-1 truncate">{entity.label}</span><StatusBadge status={entity.status} /></button>)}</CardContent></Card>
      </div>
    </div>
  </div>
}
