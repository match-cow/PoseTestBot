...
export function CaptureGate({ intent = "legacy", readiness }: CaptureGateProps = {}) {
  ...
  const activeCapture = recoveredActiveCapture ?? (submittedCapturePending
    ? {
        ...
        name: intent === "dataset" ? "Dataset capture" : intent === "calibration" ? "Calibration capture" : "Physical capture",
  ...
  const capture = useMutation({
    ...
    onSuccess: (data) => { setSubmittedCapture({ runRoot: selectedRun, jobId: data.job_id }); toast.success(copy.queued, { description: `Job ${data.job_id} continues after navigation; status, logs, and stop controls are available in Jobs.` }); setOpen(false); setRobotAck(false); setCameraAck(false); queryClient.invalidateQueries({ queryKey: ["jobs"] }); queryClient.invalidateQueries({ queryKey: ["capture-jobs", selectedRun] }); queryClient.invalidateQueries({ queryKey: ["overview", selectedRun] }); queryClient.invalidateQueries({ queryKey: ["calibration", "setup", selectedRun] }) },
