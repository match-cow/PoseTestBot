...
export function CalibrationWorkflow() {
  ...
  useEffect(() => {
    ...
    setSensorKeys((current) => current.length ? current : setup.data.cameras.map((camera) => camera.sensor_key))
    setTargetId((current) => current || setup.data.saved_targets.find((target) => target.valid)?.target_id || "")
    setActiveAttemptId((current) => current ?? setup.data.latest_attempt?.attempt_id ?? null)
  }, [setup.data])
  ...
  useEffect(() => {
    const results = attempt.data?.results?.results
    if (!results) return
    setOverrides((current) => Object.keys(current).length ? current : Object.fromEntries(results.flatMap((result) => result.recommended_candidate_id ? [[result.sensor_key, result.recommended_candidate_id]] : [])))
  }, [attempt.data?.attempt_id, attempt.data?.results])
  ...
  const createAttempt = useMutation({
    ...
    onSuccess: (value) => { setActiveAttemptId(value.attempt_id); setOverrides({}); toast.success("Calibration queued", { description: `Attempt ${value.attempt_id} · job ${value.job_id}` }); queryClient.invalidateQueries({ queryKey: ["calibration", "setup", selectedRun] }); queryClient.invalidateQueries({ queryKey: ["jobs"] }) },
