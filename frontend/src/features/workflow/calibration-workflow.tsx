...
import { CalibrationArrangementCard, calibrationArrangementForSensors, effectiveCalibrationTargetMountingFrame, POSE_TEMPLATE_BASE_SUNRISE_PATH, type CalibrationMode, type CalibrationTargetMountingFrame } from "@/components/calibration-arrangement"
...
const REQUIRED_AUTO_TIMING_IMPLEMENTATION_REVISION = "constant_latency_nearest_pose_motion_lomo_fail_closed.v4"
...
export function CalibrationWorkflow({ referenceFramePath }: { referenceFramePath?: string | null }) {
  ...
  const arrangement = useMemo(() => calibrationArrangementForSensors(setup.data?.cameras ?? []), [setup.data?.cameras])
  ...
  return <div className="space-y-5" data-testid="calibration-workflow">
    <Card><CardHeader><CardTitle>Analyze this calibration recording</CardTitle><CardDescription>The physical setup is read from Workflow step 1 and the exact printed grid selected in step 2. PoseTestBot compares supported solutions, but nothing becomes reusable until you review and save it.</CardDescription></CardHeader><CardContent className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2"><fieldset className="space-y-3"><legend className="text-sm font-semibold">Cameras from this recording</legend><p className="text-xs text-muted-foreground">Select cameras within the one mounting group recorded for this run.</p><div className="space-y-2 rounded-lg border p-3">{setup.data.cameras.map((camera) => {
        ...
        <div className="space-y-5"><div className="space-y-2"><Label>Printed calibration grid from step 2 <span className="text-destructive">Required</span></Label>{selectedTarget && targetMountingMatches ? <div className="rounded-lg border border-success/30 bg-success/5 p-3"><div className="text-sm font-semibold">{selectedTarget.display_name}</div><div className="mt-1 font-mono text-[10px] text-muted-foreground">{selectedTarget.target_id}</div><p className="mt-2 text-[11px] text-muted-foreground">The analysis uses the run-owned, hash-verified copy with its {effectiveTargetMountingFrame?.replaceAll("_", " ")} mounting frame{!selectedTarget.selected_placement?.mounting_frame ? " safely resolved from its known placement" : ""}.</p></div> : <div className="rounded-lg border border-destructive/35 bg-destructive/5 p-3 text-xs"><div className="font-semibold text-destructive">{selectedTarget && !effectiveTargetMountingFrame ? "Legacy grid mounting is not recorded" : selectedTarget ? "Grid mounting does not match the camera group" : "No valid grid is bound to this run"}</div><p className="mt-1 text-muted-foreground">{selectedTarget && !effectiveTargetMountingFrame ? "This historical target remains viewable, but its unknown placement cannot be inferred from the current cameras. Return to step 2 and explicitly reselect the target arrangement before analysis." : "Return to step 2 and select the exact printed board with the physical mounting shown above. Grid choice and mounting cannot be overridden during analysis."}</p></div>}<div className="flex items-center justify-between text-xs text-muted-foreground"><span>The saved marker geometry and mounting must match the physical setup exactly.</span><Link to="/calibration-targets" className="text-primary-strong underline-offset-4 hover:underline">Review selected grid</Link></div></div><div className="space-y-2"><div className="text-sm font-semibold">Automatic extrinsic solution comparison</div><p className="text-xs leading-relaxed text-muted-foreground">PoseTestBot compares supported grid-pose and mounting-aware extrinsic methods, validates them on held-out motion, and recommends only passing results.</p><details className="rounded-lg border bg-muted/20"><summary className="cursor-pointer px-3 py-2 text-xs font-semibold">How acceptance is decided</summary><div className="border-t px-3 py-3 text-[11px] leading-relaxed text-muted-foreground" data-testid="calibration-acceptance-thresholds">Requires ≥{setup.data.solver.thresholds.min_accepted_views} accepted views and ≥{setup.data.solver.thresholds.min_motion_poses} robot poses over ≥{setup.data.solver.thresholds.min_translation_span_mm} mm / ≥{setup.data.solver.thresholds.min_rotation_span_deg}°. Camera/grid field coverage must span ≥{((setup.data.solver.thresholds.min_image_centroid_x_span_ratio ?? 0.45) * 100).toFixed(0)}% of image width and ≥{((setup.data.solver.thresholds.min_image_centroid_y_span_ratio ?? 0.35) * 100).toFixed(0)}% of image height with ≥{((setup.data.solver.thresholds.min_image_centroid_hull_area_ratio ?? 0.10) * 100).toFixed(0)}% supported centroid-hull area; each extreme needs ≥{setup.data.solver.thresholds.image_coverage_tail_support_views ?? 5} views. The 3 × 3 centroid-cell count remains diagnostic. Each grid view needs ≥{setup.data.solver.thresholds.min_pnp_common_inliers} supported corners across at least {setup.data.solver.thresholds.min_pnp_supported_markers} markers and a whole-grid error ≤{setup.data.solver.thresholds.max_pnp_all_point_mean_reprojection_error_px} px. Camera and robot timestamps must be within {setup.data.solver.thresholds.max_nearest_pose_delta_ms} ms. A new OpenCV lens estimate separately needs ≥{setup.data.solver.thresholds.min_coverage_cells} of 9 image regions, held-out proof, ≤{setup.data.solver.thresholds.max_per_view_reprojection_error_px} px per view, and ≤{setup.data.solver.thresholds.max_intrinsic_rms_reprojection_error_px} px RMS.</div></details></div></div>
      </div>
    ...
    {attempt.data?.results && <div className="space-y-4" data-testid="calibration-results"><div><h2 className="text-xl font-semibold">Review calibration results</h2><p className="mt-1 text-sm text-muted-foreground">{attempt.data.results.recommended_camera_count} of {attempt.data.results.results.length} selected cameras passed. A multi-camera calibration is saved only when every selected camera belongs to one consistent passing solution bundle.</p></div>{attempt.data.read_only && <div className="rounded-lg border border-warning/40 bg-warning/5 p-4 text-xs" data-testid="historical-calibration-read-only"><div className="font-semibold">Historical attempt · inspection only</div><p className="mt-1 text-muted-foreground">These immutable results remain available for scientific review, but their retired request or timing contract cannot be rerun, changed, or promoted. Create a new attempt from the preserved run data.</p></div>}{attempt.data.results.results.map((result) => <CameraResultCard key={result.sensor_key} mode={attempt.data.request.mode} result={result} intrinsicComparison={attempt.data?.intrinsic_comparison?.sensors.find((item) => item.sensor_key === result.sensor_key) ?? null} selectedCandidateId={overrides[result.sensor_key] ?? ""} readOnly={attempt.data.read_only} onSelect={(candidateId) => setOverrideSelection({ runRoot: selectedRun, attemptId: activeAttemptId ?? "", values: { ...overrides, [result.sensor_key]: candidateId } })} />)}<Card className="border-primary/30"><CardContent className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-semibold">{attempt.data.read_only ? "Historical calibration cannot be saved" : "Save selected camera calibrations"}</div><div className="mt-1 text-xs text-muted-foreground">{attempt.data.read_only ? "Create a current attempt from this run's preserved evidence to produce a promotable calibration." : `${passingSelections} of ${attempt.data.results.results.length} camera profiles selected. Saving is an atomic, auditable promotion; calculation results remain immutable. The saved profiles retain the time-alignment offsets and evidence shown above.`}</div>{attempt.data.promotion && ["queued", "running"].includes(attempt.data.promotion.status) && <div className="mt-3 flex flex-col gap-3 rounded-lg border border-primary/30 bg-primary/5 p-3 text-xs sm:flex-row sm:items-center sm:justify-between" data-testid="calibration-promotion-job-status"><p><span className="font-semibold">Calibration saving is {attempt.data.promotion.status}.</span> This background work continues after navigation{attempt.data.promotion.job_id ? <> in job <span className="font-mono">{attempt.data.promotion.job_id}</span></> : null}; Jobs shows its live status and output.</p><Button asChild size="sm" variant="outline" className="shrink-0"><Link to="/jobs">Open Jobs</Link></Button></div>}{attempt.data.promotion?.status === "failed" && <div className="mt-2 text-xs text-destructive">{attempt.data.promotion.error}</div>}{attempt.data.promotion?.status === "promoted" && <div className="mt-2 flex items-center gap-1 text-xs text-success"><CheckCircle2 className="size-4" />Saved {attempt.data.promotion.promoted_profile_ids?.length ?? 0} camera profile(s).</div>}</div><Button size="lg" disabled={attempt.data.read_only || passingSelections !== attempt.data.results.results.length || promote.isPending || ["queued", "running", "promoted"].includes(attempt.data.promotion?.status ?? "")} onClick={() => promote.mutate()}>{promote.isPending || ["queued", "running"].includes(attempt.data.promotion?.status ?? "") ? <LoaderCircle className="animate-spin" /> : <Save />}{attempt.data.read_only ? "Inspection only" : attempt.data.promotion?.status === "promoted" ? "Calibrations saved" : "Save selected calibrations"}</Button></CardContent></Card></div>}
  </div>
}
...
function TimeAlignmentSummary({ search }: { search: TimeOffsetSearch | null }) {
  ...
  return <Card className={search.status === "failed" ? "border-destructive/40" : hasTimingWarnings ? "border-warning/40" : ""} data-testid="calibration-time-alignment">
    ...
    <CardContent className="space-y-4">
      {!currentTimingRevision && <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-xs" data-testid="calibration-attempt-legacy-timing-revision"><div className="font-semibold text-destructive">Historical timing evidence · inspection only</div><p className="mt-1 text-muted-foreground">The immutable attempt records <span className="font-mono">{search.implementation_revision ?? "no timing revision"}</span>. It remains available for review but cannot be rerun or promoted; create a new attempt from the preserved recordings for the current fail-closed timing rule.</p></div>}
      {search.status === "failed" && <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-xs" data-testid="calibration-time-alignment-failed"><div className="font-semibold text-destructive">Auto time alignment stopped this calibration</div><p className="mt-1 text-muted-foreground">{currentTimingRevision ? "Required robot-pose timing input was missing, corrupt, or unusable. No extrinsic result was generated or saved; inspect the checks below." : "This result was decided by the recorded legacy rule. No extrinsic result was generated or saved; create a fresh attempt after restarting the backend."}</p></div>}
      {search.status === "complete" && hasTimingWarnings && <div className="rounded-md border border-warning/40 bg-warning/5 p-3 text-xs" data-testid="calibration-time-alignment-warning"><div className="font-semibold text-warning-foreground">Calibration has advisory timing evidence</div><p className="mt-1 text-muted-foreground">The automatic offset still passed every required fail-closed identification and stability check. Review the advisory nearest-pose or absolute-offset evidence together with the final residual and reprojection evidence before saving.</p></div>}
      <div className="overflow-x-auto rounded-lg border">
        <table className="min-w-[1040px] w-full text-left text-[11px]">
          ...
          <tbody>{search.sensors.map((sensor) => {
            ...
            const decision = sensor.status === "failed" ? "Time alignment rejected" : sensor.status === "applied" && warningChecks.length > 0 ? `Applied with ${warningChecks.length} warning${warningChecks.length === 1 ? "" : "s"}` : sensor.status === "applied" ? "Time offset applied" : sensor.status === "fixed_zero" ? "Fixed zero selected" : "Zero offset identified"
            const candidateLabel = sensor.status === "failed" ? `rejected ${formatSigned(sensor.candidate_robot_pose_time_offset_ms, " ms")} candidate` : sensor.status === "applied" ? "selected offset" : "identified 0 ms offset"
...
function CameraResultCard({ mode, result, intrinsicComparison, selectedCandidateId, readOnly, onSelect }: { mode: Attempt["request"]["mode"]; result: CameraResult; intrinsicComparison: IntrinsicComparison | null; selectedCandidateId: string; readOnly: boolean; onSelect: (value: string) => void }) {
