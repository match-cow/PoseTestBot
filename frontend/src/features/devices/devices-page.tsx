        ...
        {devices.map((device) => {
          ...
          return <Card data-testid="sensor-card" data-sensor-key={key} key={key} className="min-w-0 overflow-hidden">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2"><Button data-testid="sensor-preview-toggle" aria-label={`Toggle preview for ${alias?.alias || device.effective_display_name || device.display_name || device.device_id}`} aria-pressed={previewOn} variant={previewOn ? "secondary" : "outline"} disabled={!previewSupported || previewStopping || previewTransitionPending} title={previewSupported ? undefined : "Live RGB preview is unavailable for this sensor family"} onClick={() => previewOn && preview ? stopPreview.mutate(preview.job.id) : startPreview.mutate(device)}>{!previewSupported ? <><EyeOff />Unavailable</> : previewStopping ? <><EyeOff />Stopping…</> : previewOn ? <><Eye />Preview on</> : <><EyeOff />Preview off</>}</Button><Button variant="outline" title={previewBusy ? "Turn this preview off before taking a snapshot" : undefined} onClick={() => snapshot.mutate(device)} disabled={previewBusy || snapshot.isPending || PREVIEW_BUSY.has(snapshotState?.job.status ?? "")}><Camera />{PREVIEW_BUSY.has(snapshotState?.job.status ?? "") ? "Capturing…" : "Snapshot"}</Button></div>
            </CardContent>
          </Card>
        })}
      </div>}
