...
export function DevicesPage() {
  ...
  const saveAliases = useMutation({
    ...
    onSuccess: () => { toast.success("Device labels and mounts saved"); queryClient.invalidateQueries({ queryKey: ["sensors"] }) },
    onError: (error) => toast.error("Device details could not be saved", { description: errorMessage(error) }),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Lab hardware" title="Devices" description="Discover and preview cameras, record operator-friendly identities, and keep manual robot controls separate from acquisition." actions={<><Button variant="outline" onClick={() => void refreshDiscovery()} disabled={status.isFetching}><RefreshCw className={status.isFetching ? "animate-spin" : ""} />Refresh discovery</Button><Button onClick={saveAllAliases} disabled={saveAliases.isPending || status.isPending || aliases.isPending || devices.length === 0}><Save />{saveAliases.isPending ? "Saving…" : "Save labels & mounts"}</Button></>} />
      <ProcessHandoff
        title="Device choices feed run setup"
        description="Save aliases, mounting modes, and orientation here. “Use in run” is a browser draft for a new setup; Workflow step 1 is where camera membership and capture settings become part of the selected run."
        to="/workflow/setup"
        action="Choose a workflow"
      />

      <Card className="border-primary/25">
        <CardHeader className="flex-row items-start justify-between gap-6"><div><CardTitle className="flex items-center gap-2"><Bot className="size-5 text-primary-strong" />KUKA LBR iiwa <HelpTip label="robot profile status">Configured means the fixed lab profile loaded successfully. This read-only status does not contact the robot or prove that Sunrise is running.</HelpTip></CardTitle><CardDescription>Manual commands are independent from camera previews and require explicit target confirmation.</CardDescription></div><StatusBadge status="configured">real lab profile</StatusBadge></CardHeader>
        ...
        {devices.map((device) => {
          ...
          return <Card data-testid="sensor-card" data-sensor-key={key} data-capture-ready={captureReady ? "true" : "false"} key={key} className="min-w-0 overflow-hidden">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label className="flex items-center gap-1" htmlFor={`mounting-${key}`}>Mounting <HelpTip label="camera mounting mode">Eye in hand moves with the robot. Static remains fixed in the workcell. Calibration and frame-graph logic use this distinction.</HelpTip></Label><Select value={alias?.mounting_mode ?? device.mounting_mode ?? "eye_in_hand"} onValueChange={(value) => updateAlias(device, { mounting_mode: value })}><SelectTrigger id={`mounting-${key}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="eye_in_hand">Eye in hand</SelectItem><SelectItem value="static">Static</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label className="flex items-center gap-1" htmlFor={`orientation-${key}`}>Orientation <HelpTip label="camera image orientation">Inverted rotates supported camera output by 180° so saved RGB and aligned depth share the physical mounting orientation.</HelpTip></Label><Select value={alias?.inverted ?? device.inverted ? "inverted" : "normal"} onValueChange={(value) => void updateOrientation(device, value === "inverted", preview)} disabled={device.sensor_type !== "realsense_d435" || previewStopping || previewTransitionPending}><SelectTrigger id={`orientation-${key}`} data-testid="sensor-orientation"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="normal">Normal</SelectItem><SelectItem value="inverted">Inverted</SelectItem></SelectContent></Select></div></div>
              <div className="flex items-center justify-between rounded-lg bg-muted/55 px-3 py-2"><div className="flex items-center gap-1"><Label className={`flex items-center gap-2 ${!captureReady && !selectedForRun ? "cursor-not-allowed text-muted-foreground" : ""}`} title={!captureReady && !selectedForRun ? readinessMessage ?? "Camera is not capture-ready" : undefined}><Checkbox data-testid="sensor-run-selection" checked={selectedForRun} disabled={!captureReady && !selectedForRun} onCheckedChange={(value) => toggleSelected(key, value === true, captureReady)} />Use in run</Label><HelpTip label="run camera draft">This browser-local selection prefills a new run setup. Save Workflow step 1 to change the selected run’s durable camera membership.</HelpTip></div><Button variant="ghost" size="sm" onClick={() => setDetail(device)}><Info />Details</Button></div>
