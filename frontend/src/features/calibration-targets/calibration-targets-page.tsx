...
export function CalibrationTargetsPage() {
  ...
  if (!status.data?.generation_available) return <div className="space-y-6">
    ...
    <div className="grid grid-cols-3 gap-4">{library.data?.bundles.map((bundle) => <Card key={bundle.target_id} className={bundle.selected ? "border-primary/40" : ""}><CardHeader><CardTitle className="text-base">{bundle.display_name ?? bundle.target_id}</CardTitle><CardDescription className="font-mono text-[10px]">{bundle.target_id}</CardDescription></CardHeader><CardContent className="space-y-3">{bundle.valid ? <><div className="text-xs text-muted-foreground">{bundle.target?.grid_size?.join(" × ")} markers · {bundle.target?.target_bounds.width_mm.toFixed(1)} × {bundle.target?.target_bounds.height_mm.toFixed(1)} mm</div><div className="flex flex-wrap gap-2"><DownloadLink bundle={bundle} artifact="source" icon={<FileJson />} /><DownloadLink bundle={bundle} artifact="target" icon={<FileJson />} /><DownloadLink bundle={bundle} artifact="pdf" icon={<FileText />} /></div><Button className="w-full" variant={bundle.selected ? "outline" : "default"} onClick={() => { setSelection(bundle); setPlacement((bundle.selected_placement?.mode as keyof typeof placementLabels) || "unknown") }}>{bundle.selected ? "Review selection" : "Select for run"}</Button></> : <div className="text-xs text-destructive">Invalid bundle: {bundle.error}</div>}</CardContent></Card>)}</div>
