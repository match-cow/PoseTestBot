...
export function PoseTemplatesPage() {
  ...
  const addChosenOrientation = () => {
    ...
    const instance: PositionedTemplateInstance = {
      ...
      pose: { x_mm: 40 + offset, y_mm: 40 + offset, rotation_deg: 0 }, object: chooser.object, orientation, preview_mesh: chooser.analysis.preview_mesh,
    }
  ...
  const orientationProjectionSpan = useMemo(() => chooser ? Math.max(.01, ...chooser.analysis.orientations.flatMap((orientation) => {
    const bounds = projectIsometricMesh(chooser.analysis.preview_mesh, orientation.source_to_placed).bounds
  ...
  return <div className="space-y-7" data-testid="pose-templates-page">
    ...
    <section className="space-y-3" aria-labelledby="arrange-template-heading"><div><div className="mb-1 text-[10px] font-bold uppercase tracking-[.16em] text-primary-strong">Step 2</div><h2 id="arrange-template-heading" className="text-lg font-semibold">Arrange selected slice contours</h2><p className="text-xs text-muted-foreground">Drag to move, use the round handle to rotate, or enter exact planar values. Arrow keys nudge 1 mm; Shift nudges 10 mm.</p></div>
      <div className="grid grid-cols-[minmax(0,1.35fr)_360px] items-start gap-4">
