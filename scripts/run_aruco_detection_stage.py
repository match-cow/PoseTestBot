...
def main() -> None:
    ...
    sensors = filter_enabled_sensor_folders(
        ...
        (
            ...
            if path.is_dir() and (path / RGB_DIR).is_dir()
        ),
    )
    if not sensors:
    ...
    try:
        ...
        for sensor in sensors:
            ...
            if args.save_images:
                artifacts[f"{sensor.name}:aruco_images"] = draw_detection_images(
                    sensor, detections
                )
        upsert_stage(
            manifest,
        ...
        upsert_stage(
            manifest, name="aruco_detection", status="failed", message=str(exc)
        )
        write_run_manifest(manifest, root)
        raise
