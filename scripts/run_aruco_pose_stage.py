...
def main() -> None:
    ...
    profiles_path = (
        Path(args.intrinsic_profiles)
        if args.intrinsic_profiles
        else root / INTRINSIC_CALIBRATION_PROFILES
    )
    ...
    try:
        ...
        for sensor in sensors:
            ...
            artifacts[f"{sensor.name}:{ARUCO_POSE_ESTIMATION}"] = (
                sensor / ARUCO_POSE_ESTIMATION
            )
        upsert_stage(
            manifest,
            name="aruco_pose",
